...
SWEP.Damage = 2
SWEP.UppercutDamageMultiplier = 7.5
SWEP.HitDistance = 40
...
function SWEP:PrimaryAttack(right)
	...
	self:UpdateNextIdle()
	self:SetNextMeleeAttack( CurTime() + 0.25 )
...
function SWEP:DealDamage()
	...
	if SERVER and IsValid( hitent ) then
		...
		if anim == "fists_uppercut" then
			...
			dmginfo:SetDamage(self.Damage * math.Rand(1, 2))
		end
	...
	if SERVER then
		if hitplayer and anim ~= "fists_uppercut" then
			self:SetCombo( self:GetCombo() + math.random(1, 2) )
		else
			self:SetCombo( 0 )
		end
	end
