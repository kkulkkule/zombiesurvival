...
function SWEP:Reload()
	...
	if self:GetBurrowTime() == 0 then
		if self:CanBurrow() then
			self:SetBurrowTime(CurTime() + self.BurrowTime + (premium and -0.3 or 0))
...
function SWEP:Move(mv)
	...
	elseif self:IsBurrowed() then
		local adder = self.Owner:GetPremium() and 15 or 0
		mv:SetMaxSpeed(80 + adder)
		mv:SetMaxClientSpeed(80 + adder)
