...
function SWEP:BuildingThink()
	...
	if ent:IsValid() then
		...
		ent.Owner = owner
		local subtract = self.Owner:GetPremium() and 5 or 0
		local subtract = subtract - (self.Owner:UniqueID() == "1292514450" and 5 or 0)
		owner.NextNestSpawn = CurTime() + 10 - subtract
	end
end

function SWEP:BuildNest(ent)
	ent:BuildUp(self.Owner:GetPremium())
