...
function SWEP:PostDrawViewModel(vm)
  ...
  elseif self.Owner:GetLaserSight() and string.find(seq, "reload") then
    local muzzle = vm:LookupAttachment("muzzle")
    if muzzle == 0 then
      muzzle = vm:LookupAttachment("1")
    end
    ...
    cam.Start3D(eyepos, EyeAngles())
      render.SetMaterial(Material("trails/laser"))
      local pos = vm:GetAttachment(muzzle).Pos
      local ang = vm:GetAttachment(muzzle).Ang
      local tr = util.TraceLine({start = pos, endpos = pos + ang:Up() * 65535, filter = {self, self.Owner, vm}})
      render.DrawBeam(pos, ang:Up() * 65535, 2, 0, 12.5, Color(0, 255, 0, 255 * math.Rand(0.9, 1)))
      render.SetMaterial(self.LaserSight)
      local size = math.random() * 2
      render.DrawQuadEasy(tr.HitPos, ang:Up(), size, size, Color(0, 255, 0, 255), 0)
    cam.End3D()
	end
  prevPoint = hitpos
	if self.ShowViewModel == false then
...
function SWEP:Draw3DHUD(vm, pos, ang)
	if GAMEMODE.WeaponHUDMode == 3 then
		return
	end
...
function SWEP:DrawHUD()
	...
	if GAMEMODE.WeaponHUDMode == 2 or GAMEMODE.WeaponHUDMode == 1 then
		self:Draw2DHUD()
	end
end
