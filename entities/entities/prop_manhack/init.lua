...
function ENT:Initialize()
	...
	if phys:IsValid() then
		...
		Constraint:SetKeyValue("angularlimit", 10)
...
function ENT:ThreadSafePhysicsCollide(data)
	...
	if ent and ent:IsValid() then
		if ent:IsPlayer() and ent:Team() == TEAM_UNDEAD and ent:Alive() and CurTime() >= (self.NextTouch[ent] or 0) then
			...
			local elobbc = ent:LocalToWorld(ent:OBBCenter())
			local td = {
				start = spos,
				endpos = ent:NearestPoint(spos),
				filter = self,
				mask = MASK_SHOT
			}
			
			td.endpos = td.endpos + (elobbc - spos) * 10
			
			local tr = util.TraceLine(td)
			
			if tr.HitGroup == HITGROUP_HEAD then
				dmg = self.HitDamage * 2
			else
				dmg = self.HitDamage
			end
			
			dmg = dmg + (self:GetVelocity() / self.MaxSpeed):Length() * 10
			...
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:AddVelocity(dir * self.BounceFleshVelocity)			end
		else
	...
	if not diddamage then
		...
		if data.Speed >= self.MaxSpeed * self.SelfDamageSpeed and ent and ent:IsWorld() and CurTime() >= self.PhysDamageImmunity then
			self:TakeDamage(math.Clamp(data.Speed * self.SelfDamageMul, 0, 30))		end
	end
end
