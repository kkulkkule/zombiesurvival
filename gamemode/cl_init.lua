...
local function CheckIdle()
	...
	if GAMEMODE:GetWaveActive() and GAMEMODE:GetWave() >= 1 and client:Alive() and !client:IsAdmin() then
		local idle_limit = 180
		if idle_limit <= 0 then idle_limit = 180 end -- networking sucks sometimes
		...
		elseif CurTime() > (idle.t + idle_limit) then
			RunConsoleCommand("say", "(AFKKicker) 오랜 시간 움직임이 없어 접속을 종료합니다.")
		...
		elseif CurTime() > (idle.t + (idle_limit / 2)) then
			-- will repeat
			chat.AddText(Color(255, 0, 0), "오랜 시간동안 움직임이 없었으므로, 잠시 후 접속을 종료합니다.")
		end
	end
end
...
function GM:PostRender()
	if self.m_ZombieVision and MySelf:IsValid() and MySelf:Team() == TEAM_UNDEAD then
		...
		if dlight then
			...
			dlight.g = 255
			dlight.b = 80
			dlight.Brightness = 3 * mulb
			dlight.Size = 300 * muls
			dlight.Decay = 900
...
function GM:_Think()
	...
	if myteam == TEAM_HUMAN then
		...
		if wep:IsValid() and wep.GetIronsights and wep:GetIronsights() then
			self.FOVLerp = math.Approach(self.FOVLerp, wep.IronsightsMultiplier or 0.6, frametime * 4 * (MySelf.FastSight and 2.25 or 1))
		elseif self.FOVLerp ~= 1 then
			self.FOVLerp = math.Approach(self.FOVLerp, 1, frametime * 5 * (MySelf.FastSight and 2.25 or 1))
		end
	...
	if LastLocalVoiceCheck + 0.1 <= curtime then
		if input.IsKeyDown(KEY_CAPSLOCK) and !MySelf.IsTalkingLocal then
			MySelf.IsTalkingLocal = true
			MySelf.localVoice = true
			RunConsoleCommand("localvoice", "1")
			RunConsoleCommand("+voicerecord", "1")
		end
		if !input.IsKeyDown(KEY_CAPSLOCK) and MySelf.IsTalkingLocal then
			MySelf.IsTalkingLocal = false
			MySelf.localVoice = false
			RunConsoleCommand("localvoice", "0")
			RunConsoleCommand("-voicerecord", "0")
		end
		LastLocalVoiceCheck = curtime
	end
  
  if LastMarked + 0.1 < curtime and MySelf:KeyDown(IN_USE) then
    RunConsoleCommand("zs_mark")
    LastMarked = curtime
  end
end
...
function GM:PlayerStartVoice(pl)
	if !IsValid(pl) or !IsValid(MySelf) or !IsValid(g_VoicePanelList) then
		return
	end
	GAMEMODE:PlayerEndVoice(pl, true)
	...
	local voicepanel = g_VoicePanelList
	...
	pnl.Paint = function(s, w, h)
		...
		draw.RoundedBox(4, 0, 0, w, h, self.Col or Color(0, 255 * s.ply:VoiceVolume(), 0, 240))
	end
	...
	pnl.Think = function(self)
		...
		if PlayerVoicePanels[self.ply] and PlayerVoicePanels[self.ply].localVoice then
			self.Col = Color(math.Clamp(255 * self.ply:VoiceVolume(), 200, 255), 0, 0, 240)
		else
			self.Col = Color(0, 255 * self.ply:VoiceVolume(), 0, 240)
		end
	end
...
function GM:PlayerEndVoice(pl)
	if IsValid(PlayerVoicePanels[pl]) then
		PlayerVoicePanels[pl]:Remove()
	end
end

net.Receive("localvoice", function(len)
	...
	local pl = player.GetByID(entid + 1)
	if IsValid(pl) then
		pl.localVoice = state
		if IsValid(PlayerVoicePanels[pl]) then
			PlayerVoicePanels[pl].localVoice = state
		end
	end
end)
...
function GM:PlayBeats(teamid, fear)
	...
	if LASTHUMAN and cv_ShouldPlayMusic:GetBool() then
		if self.WaveMusic then
			self.WaveMusic:FadeOut(5)
		end
		if !self.LastHumanMusic or !self.LastHumanMusic:IsPlaying() then
			local sounds = file.Find("sound/zombiesurvival/lasthuman/*.wav", "GAME")
			chat.AddText(sound)
			if #sounds >= 1 then
				local sound = "zombiesurvival/lasthuman/" .. table.Random(sounds)
				self.LastHumanMusic = CreateSound(LocalPlayer(), sound)
...
function GM:HumanMenu()
	...
	panel:AddItem(b)
	b = EasyButton(panel, "탄창 5개와 함께 무기 주기", 8, 4)
...
function GM:_CreateMove(cmd)
	...
	if MySelf:GetLegDamage() >= 0.5 + (MySelf:Team() == TEAM_ZOMBIE and -0.3 or 0) then
...
function GM:_PrePlayerDraw(pl)
	...
	local PremiumRange = (MySelf:GetPremium() and 512 or 0)

	if self.m_ZombieVision and MySelf:Team() == TEAM_UNDEAD and pl:Team() == TEAM_HUMAN and pl:GetPos():Distance(EyePos()) <= pl:GetAuraRange() + PremiumRange then
...
net.Receive("zs_wavestart", function(length)
	...
	local music = string.lower(net.ReadString())
...
net.Receive("zs_wavemusic", function(length)
	local onoff = net.ReadBit()
	local music = net.ReadString()
	local next = net.ReadFloat()
	if GAMEMODE.LastHumanMusic and GAMEMODE.LastHumanMusic:IsPlaying() then
		return
	end
	if tobool(onoff) == true then
...
net.Receive("zs_mark_start", function(len)
  if !IsValid(MySelf) then
    return
  end
  if MySelf:Team() == TEAM_ZOMBIE then
    return
  end
  local idx = net.ReadInt(8)
  local uniq = net.ReadInt(8)
  local delay = net.ReadInt(8)
  local marktime = net.ReadInt(8)
  local cooltime = net.ReadInt(8)
  local premium = tobool(net.ReadBit())
  local curtime = CurTime()
  
  local zb = player.GetByID(idx)
  if !IsValid(zb) then
    hook.Remove("HUDPaint", "ZS_MARK_START" .. tostring(idx))
    timer.Destroy("DestroyScoutHalo" .. tostring(idx))
    return
  end
  hook.Remove("HUDPaint", "ZS_MARK_START" .. tostring(idx))
  hook.Add("HUDPaint", "ZS_MARK_START" .. tostring(idx), function()
    if !zb:Alive() then
      hook.Remove("HUDPaint", "ZS_MARK_START" .. tostring(idx))
      timer.Destroy("DestroyScoutHalo" .. tostring(idx))
    end
    local ratio = 1 - math.Clamp(((curtime + delay) - CurTime()) / delay, 0, 1)
    halo.Render({
      Ents = {zb},
      Color = (premium and Color(0, 128 * (1 - ratio), 127 * ratio + 128, 255 * ratio) or Color(127 * ratio + 128, 128 * (1 - ratio), 0, 255 * ratio)),
      BlurX = (ratio == 1 and 4 or 1),
      BlurY = (ratio == 1 and 4 or 1),
      DrawPasses = 1,
      Additive = true,
      IgnoreZ = false
    })
  ...
  timer.Destroy("DestroyScoutHalo" .. tostring(idx))
  timer.Create("DestroyScoutHalo" .. tostring(idx), marktime + delay, 1, function()
    hook.Remove("HUDPaint", "ZS_MARK_START" .. tostring(idx))
  end)
end)
