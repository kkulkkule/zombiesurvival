...
function meta:NearArsenalCrate()
	...
	for _, ent in pairs(ents.FindByClass("prop_arsenalcrate")) do
		...
		if pos:Distance(nearest) <= dist and (WorldVisible(pos, nearest) or self:TraceLine(dist).Entity == ent) then
			return true
		end
	end
...
function meta:ResetSpeed(noset, health)
	...
	if 32 < speed and not GAMEMODE.ZombieEscape then
		...
		if health < 60 then
			speed = math.max((self.Willpower and 128 or 88), speed - speed * (self.Willpower and 0.25 or 0.4) * (1 - health / 60))
		end
	end
