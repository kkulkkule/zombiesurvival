...
function MakepWorth()
	...
	local savebutton = EasyButton(nil, "이 카트 저장하기", 0, 10)
	...
	for i, savetab in ipairs(GAMEMODE.SavedCarts) do
		...
		checkbutton:SetTooltip("이 저장된 목록대로 구매합니다.")
		...
		loadbutton:SetTooltip("이 저장된 목록을 불러옵니다.")
		...
		if cartname == defaultcart then
			defaultbutton:SetTooltip("즐겨찾기에서 해제합니다.")
		else
			defaultbutton:SetTooltip("즐겨찾기로 등록합니다.")
		end
		...
		deletebutton:SetTooltip("이 구매 목록을 삭제합니다.")
	...
	local worthlab = EasyLabel(frame, "자금: "..tostring(WorthRemaining), "ZSHUDFontSmall", COLOR_LIMEGREEN)
