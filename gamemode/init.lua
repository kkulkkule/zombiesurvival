...
function GM:AddResources()
	...
	resource.AddFile("sound/mysounds/weapons/c4/yolololololololo.mp3")
	...
	for _, v in pairs(file.Find("sound/zombiesurvival/lasthuman/*.wav", "GAME")) do
		resource.AddFile("sound/zombiesurvival/lasthuman/" .. v)
	end
	
	for _, v in pairs(file.Find("sound/zombiesurvival/wavemusic/*.mp3", "GAME")) do
	...
	resource.AddFile("sound/zombiesurvival/sfx/lowhealth/heartbeat-01a.wav")
	resource.AddFile("sound/zombiesurvival/sfx/lowhealth/heartbeat-02a.wav")
end
	...
	if not bossplayer then
		...
		for _, ent in pairs(team.GetPlayers(TEAM_UNDEAD)) do
			if ent:GetZombieClassTable().Boss and ent:Alive() then
				...
				if livingbosses >= 5 then return end
			else
...
function GM:Think()
	...
	if not self.RoundEnded then
		...
		if WAVESTARTMUSICNEXT <= time and self:GetWaveActive() then
			local music = table.Random(WAVESTARTMUSICS)
			WAVESTARTMUSICNEXT = time + WAVESTARTMUSICD[music]
			net.Start("zs_wavemusic")
				net.WriteBit(true)
	...
	for _, pl in pairs(zombies) do
		if pl.PlayerBoost then
			local classspeed = pl:GetZombieClassTable().Speed
			local speed = classspeed + pl.PlayerBoost.CurrentSpeed
			pl:SetMaxSpeed(speed)
			pl:SetWalkSpeed(speed)
			pl.PlayerBoost.CurrentSpeed = math.max(0, pl.PlayerBoost.CurrentSpeed - (pl.PlayerBoost.MaxSpeed * (FrameTime() / pl.PlayerBoost.Duration)))
			if pl.PlayerBoost.Until <= time then
				pl.PlayerBoost = nil
				pl:SetWalkSpeed(classspeed)
				pl:SetMaxSpeed(classspeed)
			end
		end
	end
		...
		for _, pl in pairs(humans) do
			if pl:Alive() then
				...
				if self:GetWave() >= 1 then
					...
					if pl.BuffRegenerative and time >= pl.LastBuffRegen + 4 then
...
function GM:PlayerInitialSpawnRound(pl)
	...
	pl.Blueprint = nil
	pl:SetTacticalLight(false)
	pl:SetLaserSight(false)
    pl:SetCompensator(false)
	pl:SetHunterCharge(false)
	pl.SoB = nil
	pl.Willpower = nil
	pl.AutoReload = nil
	pl.EagleEye = nil
		...
		if attacker:IsPlayer() then
			...
			if myteam ~= otherteam then
				...
				if myteam == TEAM_UNDEAD then
					...
					if attacker.LifeHumanDamage >= 50 then
				...
				elseif otherteam == TEAM_UNDEAD then
					...
					if (not victim.m_LastWaveStartSpawn or CurTime() >= victim.m_LastWaveStartSpawn + 3)
						...
						if victim.Marked and IsValid(victim.Marker) and attacker != victim.Marker then
							if victim.Marker.markUpgraded then
								victim.Marker.m_PointQueue = victim.Marker.m_PointQueue + damage * (victim:GetZombieClassTable().Boss and 0.015 or 0.008)
							end
						end
					end
...
function GM:PostHumanKilledZombie(pl, attacker, inflictor, dmginfo, assistpl, assistamount, headshot)
	pl.DeathStreak = (pl.DeathStreak or 0) + 1
end
...
function GM:PlayerBoost(pl, speed, time)
	local curtime = CurTime()
	if pl.PlayerBoost and pl.PlayerBoost.Until > curtime then
		return
	end
	pl.PlayerBoost = {
		MaxSpeed = speed,
...
function GM:ProcessDeathStreak(pl)
	local ds = pl.DeathStreak or 0
	if ds == 0 then
		pl.ds_dmgMul = 1
		pl.ds_spdMul = 1
	end
...
function GM:PremiumSpawned(pl)
	if !pl:GetPremium() then
		return
	end
	local plteam = pl:Team()
	local classname = string.lower(pl:GetZombieClassTable().Name)
	local curtime = CurTime()
	if classname == "zombie" then
		self:PlayerBoost(pl, 100, 10)
	elseif classname == "nightmare" then
		self:PlayerBoost(pl, 100, 15)
	elseif classname == "poison zombie" then
...
concommand.Add("localvoice", function(sender, command, arguments)
	if tonumber(arguments[1]) == 1 then
		sender.localVoice = true
	else
		sender.localVoice = false
	end
	net.Start("localvoice")
		net.WriteFloat(sender:EntIndex() - 1)
		net.WriteBit(sender.localVoice or false)
	net.Broadcast()
end)

local MarkDelay = CreateConVar("zs_markdelay", 2, FCVAR_SERVER_CAN_EXECUTE, "Marking delay of scout."):GetInt()
local MarkCooltime = CreateConVar("zs_markcooltime", 0, FCVAR_SERVER_CAN_EXECUTE, "Marking cooltime of scout."):GetInt()
local MarkTime = CreateConVar("zs_marktime", 10, FCVAR_SERVER_CAN_EXECUTE, "Marking time."):GetInt()
local CachedMarkedIndex = CachedMarkedIndex or {}
concommand.Add("zs_mark", function(pl, cmd, args)
  if pl.scout and pl:Alive() and pl:Team() == TEAM_HUMAN then
    if pl.LastUseMarking + (pl.markUpgraded and MarkCooltime or 1) < CurTime() then
      local td = util.GetPlayerTrace(pl)
      td.filter = {pl}
      td.mask = MASK_SHOT
      table.Add(td.filter, team.GetPlayers(TEAM_HUMAN))
      local target = util.TraceLine(td).Entity
      if !IsValid(target) or target:IsWorld() or !target:IsPlayer() or target:Team() == TEAM_HUMAN or target.Marked or target.Marking then
        net.Start("zs_mark_failed")
        net.Send(pl)
        return
      end
      ...
      net.Start("zs_mark_start")
        net.WriteInt(targetIdx, 8)
        net.WriteInt(pl:EntIndex(), 8)
        net.WriteInt(MarkDelay, 8)
        net.WriteInt(MarkTime, 8)
        net.WriteInt(MarkCooltime, 8)
        net.WriteBit(pl:GetPremium())
      net.Broadcast()
      
      if pl.markUpgraded then
        target.Marked = false
        target.Marking = false
        target.Marker = NULL
        timer.Destroy("ZS_MARKED" .. tostring(targetIdx))
        timer.Destroy("ZS_UNMARKED" .. tostring(targetIdx))
        target.Marking = true
        timer.Create("ZS_MARKED" .. tostring(targetIdx), MarkDelay, 1, function()
          target.Marked = true
          target.Marking = false
          target.Marker = pl
          table.insert(CachedMarkedIndex, targetIdx)
        end)
        timer.Create("ZS_UNMARKED" .. tostring(targetIdx), MarkTime + MarkDelay, 1, function()
          target.Marked = false
          target.Marker = NULL
          table.RemoveByValue(CachedMarkedIndex, targetIdx)
        end)
      end
    else
      net.Start("zs_mark_failed")
    ...
    pl.LastUseMarking = CurTime()
  end
end)


net.Receive("afkkick", function(len, pl)
	if pl:IsAdmin() or pl:IsBot() or pl:SteamID() == "BOT" then
		return
	end
